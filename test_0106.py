...
def train_experiment(
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(df_shuffled), batch_size):
            ...
            h_w = get_embedding(model, chosen, train=True)
            h_l = get_embedding(model, rejected, train=True)
            ...
            if use_contrastive:
                ...
                with torch.no_grad():
                    h_w_ref = get_embedding(model, chosen, train=True)
                    h_l_ref = get_embedding(model, rejected, train=True)

                L_cont = 0.5 * (
                    contrastive_loss(h_w_ref.detach(), h_w) +
                    contrastive_loss(h_l_ref.detach(), h_l)
                )
            else:
